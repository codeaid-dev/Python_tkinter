...
def calc():
    ...
    if len(ratio) != 2:
        ...
        result_label.config(text='有効なアスペクト比を入力してください。')
        result_label.update()
        return
    try:
        w_ratio = int(ratio[0])
        ...
        result_label.config(foreground='red',background=root.cget('bg'))
        result_label.config(text='アスペクト比は整数で入力してください。')
        result_label.update()
        return
    ...
    if iv.get() == 1:
        ...
        try:
            ...
            result_label.config(foreground='red',background=root.cget('bg'))
            result_label.config(text='幅に有効な整数を入力してください。')
            result_label.update()
            return
        result_label.config(fg=default_color)
        result_label.config(text='ここにエラーを表示')
        ...
        try:
            ...
            result_label.config(foreground='red',background=root.cget('bg'))
            result_label.config(text='高さに有効な整数を入力してください。')
            result_label.update()
            return
        result_label.config(fg=default_color)
        result_label.config(text='ここにエラーを表示')
