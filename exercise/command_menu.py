...
def file_open():
    cvs.delete('all')
    cvs.create_text(250,100,text="ファイルを開く",fill='black',font=('Helvetica',20))

def file_save():
    cvs.delete('all')
    cvs.create_text(250,100,text="ファイルを保存",fill='black',font=('Helvetica',20))

def displayed():
    cvs.delete('all')
    cvs.create_text(250,100,text=f"表示1:{bv1.get()}, 表示2:{bv2.get()}, 表示3:{bv3.get()}",fill='black',font=('Helvetica',20))

def selected():
    cvs.delete('all')
    cvs.create_text(250,100,text=f"{iv.get()}を選択",fill='black',font=('Helvetica',20))
